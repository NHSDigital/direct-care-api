...
class LoggedAction:
    def __init__(
        ...
        **fields,
        ...
        self.fields = {}  # type: ignore [var-annotated]
    ...
    def add_fields(self, **fields):
        ...
        self.fields.update(
            {
                k: v if not v or not hasattr(v, "to_primitive") else v.to_primitive()
    ...
    def serialize_for_broadcast(self, **other_args):
        ...
        to_broadcast[Constants.LOG_AT_LEVEL] = logging.getLevelName(
            logging_context.log_at_level()
        )
        return to_broadcast
...
class _ThreadLocalContextStorage(threading.local):
    def __init__(self):
        self._stack: List[LoggedAction] = []  # type: ignore [annotation-unchecked]
        self._global_fields: Dict[str, Any] = {}  # type: ignore [annotation-unchecked]
        self._force_log_at_levels: List[int] = []  # type: ignore [annotation-unchecked]
...
class _TaskIsolatedContextStorage:
    ...
    def _get_task_store(self):
        ...
        if task not in self._task_store:
            self._task_store[task] = {"stack": [], "globals": {}, "forced_levels": []}

        return self._task_store[task]

    @property
    def stack(self) -> Optional[List[LoggedAction]]:
        return self._get_task_store()["stack"]  # type: ignore [no-any-return]

    @property
    def global_fields(self) -> Optional[Dict[str, Any]]:
        return self._get_task_store()["globals"]  # type: ignore [no-any-return]

    @property
    def forced_log_levels(self) -> List[int]:
        return self._get_task_store()["forced_levels"]  # type: ignore [no-any-return]
...
class _LoggingContext(threading.local):
    ...
    def __init__(
        self,
        storage: Union[
            _TaskIsolatedContextStorage, _ThreadLocalContextStorage, None
        ] = None,
    ):
        self._storage = storage
        self._exception_extractors: List[Any] = []
    ...
    def force_log_at_level(self, log_at_level: Union[int, str]):
        ...
        log_level = (
            logging.getLevelName(log_at_level)
            if isinstance(log_at_level, str)
            else log_at_level
        )
    ...
    def log_at_level(self) -> int:
        if self.storage.forced_log_levels:
            return int(self.storage.forced_log_levels[-1])

        return int(app_logger.log_at_level)
    ...
    def pop(self) -> LoggedAction:
        ...
        Returns:
            LoggedAction
        """
        return self.storage.stack.pop(-1)  # type: ignore [no-any-return]
    ...
    def current(self) -> Union[LoggedAction, None]:
        ...
        if not self.storage.stack:
            return None
        return self.storage.stack[-1]  # type: ignore [no-any-return]
    ...
    def emit_message(self, message):
        ...
        final_message[Constants.TIMESTAMP_FIELD] = final_message.get(
            Constants.TIMESTAMP_FIELD, time()
        )
        ...
        app_logger.log(
            log_level, exc_info, args=lambda: final_message, caller_info=caller_info
        )

    def log_exception(self, **kwargs):
        app_logger.exception(**kwargs)
    ...
    def start_action(
        ...
        **fields,
        ...
        If register_global is true or omitted, the new action is pushed
        onto the global logger stack.
        This will mean it will be the new "current action"
        and accessible via the global "add_field" function.

        If register_global is false, the action is not pushed onto the stack and is only accessible
        via its own methods.

        It is recommended to set register_global to False
        when starting an action within a generator.

        Args:
            register_global (Optional[bool]): True if this action
                should be pushed onto the global stack
    ...
    def handle_action_complete(
        self, action: LoggedAction, exec_info: Union[Tuple[type, Exception, Any], None]
    ):
        ...
        message[Constants.EXECUTION_SECONDS_FIELD] = message.get(
            Constants.EXECUTION_SECONDS_FIELD,
            float(f"{action.end_time - action.start_time}"),  # type: ignore [operator]
        )

        if exec_info:
            message[Constants.MESSAGE_TYPE_FIELD] = message.get(
                Constants.MESSAGE_TYPE_FIELD, Constants.TRACEBACK_FIELD
            )
            message[Constants.EXEC_INFO_FIELD] = exec_info

        message[Constants.ACTION_STATUS_FIELD] = (
            Constants.FAILED_STATUS
            if exec_info is not None
            else Constants.SUCCEEDED_STATUS
        )
